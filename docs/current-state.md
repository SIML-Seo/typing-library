...
- `typing`은 집중형 레이아웃 기준으로 오버레이 입력, 즉시 오타 표시, 문단 종료 오타 상세 리포트(구간/예상값/입력값), 로컬 자동 저장/이어하기, 완료 결과 로컬 저장, 설정 패널(폰트/테마/판정/오타 표시 방식 + 화면 필터 + 타건 사운드)까지 연결됨
- `results`는 로컬에 저장된 완료 세션을 조회하는 히스토리 화면까지 구현됨(검색 + 공개/내 작품 필터)
- `my-works`는 `.txt` 업로드/텍스트 붙여넣기 기반 로컬 원고 저장, 삭제, 필사 시작까지 구현됨
